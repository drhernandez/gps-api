...
//    @Test
//    public void checkAccessTokenTest_error(){
//        AccessTokens token = mock(AccessTokens.class);
//
//        Mockito.when(usersDao.fetchOneByEmail(any())).thenReturn(mock(Users.class));
//        Mockito.when(accessTokensDao.fetchOneByUserId(any())).thenReturn(null);
//        Mockito.when(authService.createAccessToken(any())).thenReturn(token);
//
//        ResponseDTO response = authService.checkAccessToken(mock(CredentialsDTO.class));
//        assertEquals(response.getModel(), token);
//
//    }
}
